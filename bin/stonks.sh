...
function draw_stock()
{
  ...
  : '
    # Get symbol to draw by comparing to last position
    if [[ "$lastposy" -gt "$posy" ]]; then
      sym="/"
    elif [[ "$lastposy" -lt "$posy" ]]; then
      sym="\\"
    else
      sym="-"
    fi
    lastposy=$posy
    # Mark actual position in coordinate system
    tput cup "$posy" $((xo+i))
    printf "%s" "$sym"
  '
  done
...
# TODO: convert to generic key event handler
